...
// SnapshotBoard
// 报表中心 6 个详情页复用的「驾驶舱视图」：4 KPI + 趋势图 + 品牌排名。
// 数据来源与 dashboard 一致（/api/dashboard/snapshot）；不支持真实接口时自动降级为 Mock。
// 设计风格沿用 dashboard 暗色主题（与列表页浅色风格做明确区分，避免误导）。
...
export default function SnapshotBoard({
  title = '广汽集团经营驾驶舱',
  subtitle = '报表中心复用驾驶舱视图 · 数据由 /api/dashboard/snapshot 实时聚合',
}: SnapshotBoardProps) {
      ...
      {loading && !snapshot ? (
        ...
        <>
          ...
          <div className="text-center text-xs text-slate-500 mt-6">
            <a href="/reports" className="hover:text-amber-400">← 返回报表中心</a>
